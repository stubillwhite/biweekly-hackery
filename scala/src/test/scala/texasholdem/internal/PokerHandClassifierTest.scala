...
class PokerHandClassifierTest extends AnyFlatSpec with Matchers with MockitoSugar with TestCards {
  ...
  //
  //Player hole cards:
  //  player-a: A♤ 6♣ 	 [Straight (9 high)]
  //  player-b: 4♦ Q♣ 	 [High card (K, Q, 9, 8, 7 kickers)]
  //  player-c: 9♥ 8♥ 	 [Two pair, 9s and 8s (K kicker)]
  //  player-d: K♦ 8♦ 	 [Two pair, Ks and 8s (9 kicker)]
  //
  //Hand ranks:
  //  player-d
  //  player-c
  ...
  it should "reproduce bug" in {
    val communityCards = List(D7, HK, S8, C5, S9)
    val playeraCards = List(CA, C6)
    val playerdCards = List(DK, D8)

    println(bestPokerHand(playeraCards, communityCards))
    println(bestPokerHand(playerdCards, communityCards))
  }
